...
int32_t VL53L0X_platform_wait_us(int32_t wait_us) {
    ...
    /*
     * Use macOS threading to perform non-blocking wait.
     */
    struct timespec ts;
    clock_gettime(CLOCK_REALTIME, &ts);
    ts.tv_nsec += wait_us * 1000; // Convert microseconds to nanoseconds

    // Wait on the condition using a conditional variable
...
int32_t VL53L0X_wait_ms(int32_t wait_ms) {
    ...
    ts.tv_nsec += (wait_ms % 1000) * 1000000; // Convert milliseconds to nanoseconds
